...
# app.config['MAIL_USERNAME'] = 'expensetracker'
# app.config['MAIL_PASSWORD'] = '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'
# app.config['MAIL_DEFAULT_SENDER'] = '09tinkers-hobby@icloud.com'
# mail = Mail(app)
cors = CORS(app)
...
conn = ibm_db.connect(os.getenv('DB_CONN_STRING'), "", "")
...
def sendMail():
    message = Mail(
        from_email='nivasmuthu452@gmail.com',
        to_emails='nivasmuthu452@gmail.com',
        subject='Sending with Twilio SendGrid is Fun',
        html_content='<strong>and easy to do anywhere, even with Python</strong>')
    sg = SendGridAPIClient(os.getenv('SEND_GRID_SECRET'))
...
def check_monthly_limit(month, year):
    ...
    if amt_spent and monthly_limit and int(amt_spent[0]) > int(monthly_limit[0]):
        ...
        message = Mail(
            ...
            to_emails='nivasmuthu452@gmail.com',
            subject='Sending with Twilio SendGrid is Fun',
            html_content=f'Monthly limit exceeded by {diff} for the month of {month}, {year}')
        sg = SendGridAPIClient(os.getenv('SEND_GRID_SECRET'))
        response = sg.send(message)
        print(response.status_code)
        print(response.body)
